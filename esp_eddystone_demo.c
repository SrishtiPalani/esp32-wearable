...
static int ble_ranging(int rssi)
{
    if(rssi <= -76 && rssi >= -120){
        //3 is far
        gpio_set_level(SPKR_0, 1);
        gpio_set_level(SPKR_1, 1);
        gpio_set_level(SPKR_2, 0);
        ...
        //2 is near
        gpio_set_level(SPKR_0, 1);
        gpio_set_level(SPKR_2, 1);
        gpio_set_level(SPKR_1, 0);

        gpio_set_level(MTR_1, 1);
        vTaskDelay(100 / portTICK_RATE_MS);
        gpio_set_level(MTR_1, 0);
        return 2;
    }
    else if(rssi >= -50){
        //1 is immediate
        gpio_set_level(SPKR_1, 1);
        gpio_set_level(SPKR_2, 1);
        gpio_set_level(SPKR_0, 0);

        gpio_set_level(MTR_0, 1);
        vTaskDelay(100 / portTICK_RATE_MS);
        gpio_set_level(MTR_0, 0);
        return 1;
    }
    else{
        //0 means out of range or not detected
        gpio_set_level(SPKR_1, 1);
        gpio_set_level(SPKR_2, 1);
        gpio_set_level(SPKR_0, 1);
        //vTaskDelay(1000 / portTICK_RATE_MS);
...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t* param)
{
    switch(event)
    {
        case ESP_GAP_BLE_SCAN_PARAM_SET_COMPLETE_EVT: {
            //duration in seconds and 0 means scanning permanently
            uint32_t duration = 0;
        ...
        case ESP_GAP_BLE_SCAN_START_COMPLETE_EVT: {
            if(param->scan_start_cmpl.status != ESP_BT_STATUS_SUCCESS) {
                ESP_LOGI(DEMO_TAG,"Scan start failed");
            }
            else {
            ...
            switch(scan_result->scan_rst.search_evt)
            {
                case ESP_GAP_SEARCH_INQ_RES_EVT: {
                    ...
                    if (ret) {
                        ...
                        ESP_LOGI(DEMO_TAG, "Proximity of packet:%d", ble_ranging(scan_result->scan_rst.rssi));
        ...
        case ESP_GAP_BLE_SCAN_STOP_COMPLETE_EVT:{
            if(param->scan_stop_cmpl.status != ESP_BT_STATUS_SUCCESS) {
                ESP_LOGI(DEMO_TAG,"Scan stop failed");
            }
            else {
...
{
    //make the pin (SPKR_X) a GPIO Pin (if not default)
    gpio_pad_select_gpio(SPKR_0);
    gpio_pad_select_gpio(SPKR_1);
    gpio_pad_select_gpio(SPKR_2);

    //make the pin (SPKR_X) an output pin
    gpio_set_direction(SPKR_0, GPIO_MODE_OUTPUT);
    gpio_set_direction(SPKR_1, GPIO_MODE_OUTPUT);
    gpio_set_direction(SPKR_2, GPIO_MODE_OUTPUT);

    //initialize the value
    //For speaker low (GND) is on and high is off
    gpio_set_level(SPKR_0, 1);
    gpio_set_level(SPKR_1, 1);
    gpio_set_level(SPKR_2, 1);
}
...
void mtr_setup(void)
{
    gpio_pad_select_gpio(MTR_0);
    gpio_pad_select_gpio(MTR_1);

    gpio_set_direction(MTR_0, GPIO_MODE_OUTPUT);
    gpio_set_direction(MTR_1, GPIO_MODE_OUTPUT);
